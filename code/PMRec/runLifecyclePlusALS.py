...
def run(param, data_dict=None):
    ...
    if not has_data:
        ...
        for act in activity_lc_list:
            possible = filter(lambda s: s.split('->')[0]==act, step_dict_lc[2])
...
if __name__ == '__main__':
    ...
    feature_list = ['step_lc_plus']
    n_iter_list = [0]
    rank_list = [150]
    # parameters and regularization for als model
    init_stdev_list = [.1]
    l2_reg_w_list = [.5]
    l2_reg_V_list = [.1]
    l2_reg_list = [0]
    ...
    for log_dir, stepsz, negative_sample in configs_major:
        ...
        for feature, rank, n_iter, init_stdev, l2_reg_w, \
            ...
            result_dir = base_dir + os.sep + 'stepLcPlusWFullALS150' + os.sep + \
