...
def partialcase_to_fm_format(caseid, partialcase, case_amt, stepsz, cid_list, \
                             amt_list, step_dict, next_step_mapping, \
    ...
    x_shape[1] = cid_list.shape[0] + all_steps + \
            amt_list.shape[0] + step_dict[2].shape[0]
    ...
    amt_repeat = np.asarray([case_amt,]).repeat(len(samples))
    amt_datalist, amt_row_inds, amt_col_inds, amt_shape = \
            rutils.single_to_fm_format(amt_repeat, amt_list)
...
#    print('Samples: {}'.format(samples))
    ...
    # shift next columns by |cid_list| + |step_list| + |amt_list|
...
def case_to_fm_format(caseid, case, case_amt, stepsz, cid_list, \
                      amt_list, step_dict, \
...
def log_to_fm_format(log, stepsz, cid_list, amt_list, step_dict, next_step_mapping, \
...
#        print('Building for case: {}'.format(cid))
        ...
        case_amt = case_df['amount_req'].unique()
        assert case_amt.shape[0] == 1
        case_amt = case_amt.ravel()[0]
        x_datalist_i, x_row_inds_i, x_col_inds_i, x_shape_i, \
                y_datalist_i, pred_id_list_i = \
                case_to_fm_format(cid, case, case_amt, stepsz, cid_list, \
                                  amt_list, step_dict, next_step_mapping, \
