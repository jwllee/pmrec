...
def partialcase_to_fm_format(caseid, partialcase, case_impact, stepsz, cid_list, \
    ...
    amt_repeat = np.asarray([case_impact,]).repeat(len(samples))
...
def case_to_fm_format(caseid, case, case_impact, stepsz, cid_list, \
                      impact_list, step_dict, \
...
def log_to_fm_format(log, stepsz, cid_list, impact_list, step_dict, next_step_mapping, \
...
#        print('Building for case: {}'.format(cid))
        ...
        case_impact = case_df['impact'].unique()
        case_impact = filter(lambda item: not pd.isnull(item), case_impact)
        case_impact = np.asarray(list(case_impact))
        assert case_impact.shape[0] == 1
        case_impact = case_impact.ravel()[0]
        x_datalist_i, x_row_inds_i, x_col_inds_i, x_shape_i, \
                y_datalist_i, pred_id_list_i = \
                case_to_fm_format(cid, case, case_impact, stepsz, cid_list, \
                                  impact_list, step_dict, next_step_mapping, \
