...
def partialcase_to_fm_format(caseid, partialcase_lifecycle, \
                             stepsz, cid_list, \
                             step_dict_lifecycle, \
                             next_step_mapping, \
    ...
    if partialcase_lifecycle.shape[0] <= 2:
    ...
    x_shape[1] = cid_list.shape[0] + all_steps + step_dict_lifecycle[2].shape[0]
    x_shape = x_shape.astype(np.int)

    # make into 2-steps
    steps = lutils.case_to_steps(partialcase_lifecycle, 2)
    partialsteps = steps[:-1]
    to_predict = steps[-1]
    possible = next_step_mapping[partialcase_lifecycle[-2]]
...
#    print('Samples: {}'.format(samples))
    next_datalist, next_row_inds, next_col_inds, next_shape = \
            rutils.single_to_fm_format(samples, step_dict_lifecycle[2])
...
def case_to_fm_format(caseid, case_lc, stepsz, cid_list, \
                      step_dict_lc, \
    ...
    if case_lc.shape[0] <= minpartialsz:
        return np.asarray(x_datalist), np.asarray(x_row_inds), \
                np.asarray(x_col_inds), x_shape, \
                np.asarray(y_datalist), np.asarray(pred_id_list)

    for ind in range(minpartialsz, case_lc.shape[0] + 1):
        partialcase_lc = case_lc[:ind]
#        print('building for partial case: {}'.format(partialcase_lc))
        x_datalist_i, x_row_inds_i, x_col_inds_i, x_shape_i, \
                y_datalist_i, pred_id_list_i = \
                partialcase_to_fm_format(caseid, \
                                         partialcase_lc, \
                                         stepsz, cid_list, \
                                         step_dict_lc, \
                                         next_step_mapping, \
...
def log_to_fm_format(log, stepsz, cid_list, \
...
#        print('Building for case: {}'.format(cid))
        ...
        x_datalist_i, x_row_inds_i, x_col_inds_i, x_shape_i, \
                y_datalist_i, pred_id_list_i = \
                case_to_fm_format(cid, case_lc, stepsz, cid_list, \
                                  step_dict_lc, next_step_mapping, \
